  ...
  return (
    <div className="min-h-screen bg-[#F8F8F8]" dir="rtl" lang="dv">
      <Helmet>
        ...
        <meta
          name="description"
          content="ރައްޔިތުންގެ ދިވެހި ޕޮޑްކާސްޓް އެޕިސޯޑްތައް."
        />
      ...
      <main className="max-w-5xl mx-auto px-4 py-12">
        <div className="border-b-2 border-[#103820] pb-3 mb-8 text-right">
          ...
          <p className="font-thaana text-[#6B756E] mt-1 text-sm">
            ރާއްޖެ، މުޖުތަމައު، ޘަޤާފަތާއި ވިޔަފާރީގެ މައުޟޫތަކާ ބެހޭ
            ވާހަކަތައް.
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="space-y-4">
            {podcasts.map((podcast, index) => (
              <article
                key={podcast.id}
                className="bg-white border border-[#E5E7E2] rounded-sm p-5 flex flex-col sm:flex-row-reverse gap-5 hover:shadow-sm transition-shadow"
              >
                {podcast.cover_image_url ? (
                  <img
                    src={podcast.cover_image_url}
                    alt={podcast.title}
                    className="w-full sm:w-24 h-40 sm:h-24 object-cover rounded-sm flex-shrink-0"
                    loading="lazy"
                  />
                ) : (
                  <div className="w-full sm:w-24 h-40 sm:h-24 bg-[#D8E8D8] rounded-sm flex-shrink-0 flex items-center justify-center">
                    <span className="font-serif text-[#103820] text-2xl font-bold">
                      {index + 1}
                    </span>
                  </div>
                )}
                <div className="flex-1 min-w-0 text-right">
                  ...
                  {podcast.description && (
                    <p className="font-thaana text-sm text-[#6B756E] leading-relaxed line-clamp-2 mb-3">
                      {podcast.description}
                    </p>
                  )}
                ...
                {podcast.audio_url ? (
                  <a
                    ...
                    className="flex-shrink-0 w-10 h-10 rounded-full bg-[#103820] flex items-center justify-center hover:bg-[#183028] transition-colors"
                    aria-label={`އަޑުއަހާ ${podcast.title}`}
                  >
          ...
          <Empty>
            <EmptyHeader>
              ...
              <EmptyTitle>
                <span className="font-thaana">ދިވެހި އެޕިސޯޑެއް ނެތް</span>
              </EmptyTitle>
              <EmptyDescription>
                <span className="font-thaana">
                  ދިވެހި ޕޯޓަލް ހޮވައި ޕަބްލިޝްކުރާ އެޕިސޯޑްތައް މިތަނުން
                  ފެންނާނެ.
                </span>
              </EmptyDescription>
            </EmptyHeader>
