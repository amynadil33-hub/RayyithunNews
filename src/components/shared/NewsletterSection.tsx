...
export default function NewsletterSection({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      toast.success(
        isDhivehi
          ? "ޝުކުރިއްޔާ! ކަލޭ ރެޖިސްޓްރީ ވެއްޖެ."
  ...
  return (
    <section className="bg-[#103820] px-3 py-10 sm:px-4 sm:py-14">
      <div className="max-w-2xl mx-auto text-center">
        <h2
          className={`font-bold text-white mb-2 ${isDhivehi ? "font-thaana thaana-headline text-xl md:text-2xl" : "font-serif text-2xl md:text-3xl"}`}
        >
          {isDhivehi ? "ސަބްސްކްރައިބް ކުރެއްވުމަށް" : "Stay Informed"}
        </h2>
        <p
          className={`text-[#95D5B2] mb-8 text-sm md:text-base ${isDhivehi ? "font-thaana thaana-body" : ""}`}
        >
          {isDhivehi
        ...
        <form
          ...
          className={`flex flex-col sm:flex-row gap-3 max-w-md mx-auto ${isDhivehi ? "sm:flex-row-reverse" : ""}`}
        >
          <input
            type="email"
            value={email}
            onChange={(e) => setEmail(e.target.value)}
            placeholder={isDhivehi ? "އީމެއިލް" : "Your email address"}
            required
